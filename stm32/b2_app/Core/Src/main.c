...
static void confirm_boot(void) {
	...
	if ((meta->boot_confirmed == BOOT_CONFIRMED) && (meta->boot_attempts == BOOT_ATTEMPTS_FRESH)) {
		return;
	}

	// copy current metadata into ram before erasing
	...
	const uint32_t *src = (const uint32_t *)&updated;
...
int main(void)
{
  ...
  while (1) {
	...
	if (!confirmed && (HAL_GetTick() > CONFIRM_AFTER_MS)) {
		confirm_boot();
		confirmed = 1U;
	}
